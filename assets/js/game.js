(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const CONTENT_FETCH_TIMEOUT_MS = 4500;
    ...
    const setHudText = (text) => {
      if (overlayUi.hud) {
        if (state.ui.debugEnabled && state.ui.debugText) {
          overlayUi.hud.textContent = `${text} | DBG: ${state.ui.debugText}`;
        } else {
    ...
    const syncGameActiveState = () => {
      ...
      state.input.active = !blockedByModal;
    ...
    const fetchContentJson = async () => {
      ...
      const pageRelative = new URL("data/content.json", window.location.href).toString();
      const originRoot = new URL("/data/content.json", window.location.origin).toString();
      ...
      for (const path of candidates) {
        try {
          logDebug("content fetch try", path);
          const payload = await fetchWithTimeout(path);
          if (!validateContentSchema(payload)) {
            throw new Error("Schema validation failed");
          }
          logDebug("content fetch success", path);
          return payload;
        } catch (error) {
          lastError = error;
          logDebug("content fetch fail", path, String(error));
        }
      }
    ...
    const getFallbackContent = () => ({
      meta: {
        siteName: "Portfolio Game",
        ownerName: "Ayodeji Olaleye",
        resumeUrl: "images/OlaleyeResume.pdf",
        social: []
      },
      locations: {
        home: {
          ...
          blurb: "Primary content could not be loaded; fallback content is active.",
          highlights: [],
          cta: []
        },
        research: {
          title: "Research",
          intro: "Fallback research summary.",
          items: []
        },
        software: {
          title: "Software",
          intro: "Fallback software summary.",
          items: []
        },
        background: {
          title: "Background",
    ...
    const update = (deltaMs, nowMs) => {
      ...
      updateUIState();
      if (state.ui.debugEnabled) {
        const held = Array.from(state.input.held).join(",") || "-";
        state.ui.debugText =
          `active=${state.input.active} held=${held} moving=${state.player.isMoving}` +
          ` tile=${state.player.tileX},${state.player.tileY} to=${state.player.toTileX},${state.player.toTileY}`;
      }
      updateHudText(nowMs);
    };
    ...
    const preventScrollIfGameActive = (event) => {
      ...
      const isInteract = isInteractEvent(event);
      if (!controlledKeys.has(code) && !controlledKeys.has(key) && !dir && !isInteract) {
        return false;
      }
      const active = event.target instanceof HTMLElement ? event.target : document.activeElement;
      if (isEditableElement(active) && active !== canvas) {
        return false;
      }
      if (!state.input.active) {
        return false;
    ...
    const getDirectionFromEvent = (event) => {
      return keyToDirection[event.code] || keyToDirection[event.key] || null;
    };

    const isInteractEvent = (event) => {
      return event.code === "Space" || event.key === " ";
    };

    const onKeyDown = (event) => {
      if (state.ui.debugEnabled) {
        state.ui.debugText = `keydown key=${event.key || "-"} code=${event.code || "-"} repeat=${event.repeat}`;
      }
      logDebug("keydown", event.key, event.code, "active=", state.input.active);

      const eventTarget = event.target instanceof HTMLElement ? event.target : null;
      if (eventTarget && isEditableElement(eventTarget) && eventTarget !== canvas) {
        return;
      }
      ...
      const interact = isInteractEvent(event);

      if (direction) {
        state.input.held.add(direction);
        logDebug("held add", direction, Array.from(state.input.held));
      }
    ...
    const onKeyUp = (event) => {
      if (state.ui.debugEnabled) {
        state.ui.debugText = `keyup key=${event.key || "-"} code=${event.code || "-"}`;
      }
      logDebug("keyup", event.key, event.code);
      ...
      const direction = getDirectionFromEvent(event);
      if (direction) {
        state.input.held.delete(direction);
        logDebug("held delete", direction, Array.from(state.input.held));
      }
    };
    ...
    const loadContentData = async () => {
      try {
        ...
        state.content.error = "";
        logDebug("content loaded");
      } catch (error) {
        const fallback = getFallbackContent();
        if (validateContentSchema(fallback)) {
          state.content.data = fallback;
          state.content.loaded = true;
          state.content.error = "";
          setToast("Using fallback content");
          logDebug("fallback content active");
          return;
        }
    ...
    document.addEventListener("keyup", onKeyUp, { passive: false, capture: true });
